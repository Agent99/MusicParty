...
{
    private readonly HttpClient _http = new HttpClient(new HttpClientHandler() { UseCookies = true });
    ...
    private string GetTimestamp() {
        Console.WriteLine("Timestamp==="+DateTimeOffset.Now.ToUnixTimeSeconds().ToString());
    ...
    public async Task<IEnumerable<Music>> SearchMusicByNameAsync(string name)
    {
        ...
        return from b in j["result"]!["songs"]!.AsArray()
            ...
            let artists = b["artists"].AsArray().Select(y => y["name"]?.GetValue<string>() ?? string.Empty).ToArray()
            select new Music(id2, name2, artists);
    }
    ...
    public async Task<IEnumerable<PlayList>> GetMusicListByName(string name, int offset = 0)
    {
        ...
        Console.WriteLine("GetMusicListByName...resp===j"+j);
        
        return from b in j["result"]!["songs"]!.AsArray()
            let id2 = b["id"].GetValue<long>().ToString()
            let name2 = (string)b["name"]
            let firstName = b["artists"].AsArray().Select(y => (string)y["name"]).FirstOrDefault()
            select new PlayList(id2, name2+"-"+firstName);
    }
